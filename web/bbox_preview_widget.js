/**
 * ComfyUI Hunyuan3D-Part - Bounding Box Preview Widget
 * Displays meshes with bounding boxes using VTK.js
 */

import { app } from "../../../scripts/app.js";

console.log("[Hunyuan3D] Loading bounding box preview extension...");

app.registerExtension({
    name: "hunyuan3d.bboxpreview",

    async beforeRegisterNodeDef(nodeType, nodeData, app) {
        if (nodeData.name === "Hunyuan3DPreviewBoundingBoxes") {
            console.log("[Hunyuan3D] Registering Preview Bounding Boxes node");

            const onNodeCreated = nodeType.prototype.onNodeCreated;
            nodeType.prototype.onNodeCreated = function() {
                const r = onNodeCreated ? onNodeCreated.apply(this, arguments) : undefined;

                console.log("[Hunyuan3D BBox] Node created, adding widget");
                ...
                iframe.src = "/extensions/ComfyUI-Hunyuan3D-Part/viewer_vtk.html?v=" + Date.now();
                ...
                const widget = this.addDOMWidget("preview_bbox", "BBOX_PREVIEW", container, {
                    getValue() { return ""; },
                    setValue(v) { }
                });
                ...
                this.bboxViewerIframe = iframe;
                this.bboxInfoPanel = infoPanel;

                // Track iframe load state
                let iframeLoaded = false;
                iframe.addEventListener('load', () => {
                    console.log("[Hunyuan3D BBox] Iframe loaded");
                    iframeLoaded = true;
                });
                ...
                window.addEventListener('message', async (event) => {
                    ...
                    if (event.data.type === 'SCREENSHOT' && event.data.image) {
                        console.log('[Hunyuan3D BBox] Received screenshot from iframe');
                        ...
                        try {
                            ...
                            const filename = `hunyuan3d-bbox-screenshot-${timestamp}.png`;
                            ...
                            console.log('[Hunyuan3D BBox] Uploading screenshot to server...');
                            ...
                            if (response.ok) {
                                ...
                                console.log('[Hunyuan3D BBox] Screenshot saved to outputs folder:', result.name);
                            } else {
                                throw new Error(`Upload failed: ${response.status} ${response.statusText}`);
                            }

                        } catch (error) {
                            console.error('[Hunyuan3D BBox] Error saving screenshot:', error);
                        }
                    }
                    // Handle error messages from iframe
                    else if (event.data.type === 'MESH_ERROR' && event.data.error) {
                        console.error('[Hunyuan3D BBox] Error from viewer:', event.data.error);
                        if (infoPanel) {
                ...
                this.onExecuted = function(message) {
                    console.log("[Hunyuan3D BBox] onExecuted called with message:", message);
                    ...
                    if (message?.mesh_file && message.mesh_file[0]) {
                        ...
                        console.log(`[Hunyuan3D BBox] Loading mesh with bboxes: ${filename}`);
                        ...
                        const sendMessage = () => {
                            if (iframe.contentWindow) {
                                console.log(`[Hunyuan3D BBox] Sending postMessage to iframe: ${filepath}, lineWidth: ${lineWidth}`);
                                ...
                                console.error("[Hunyuan3D BBox] Iframe contentWindow not available");
                            }
                        };

                        // Send message after iframe is loaded
                        if (iframeLoaded) {
                            console.log("[Hunyuan3D BBox] Iframe already loaded, sending immediately");
                            sendMessage();
                        } else {
                            console.log("[Hunyuan3D BBox] Waiting for iframe to load...");
                            setTimeout(sendMessage, 500);
                        }
                    } else {
                        console.log("[Hunyuan3D BBox] No mesh_file in message data. Keys:", Object.keys(message || {}));
                    }
                };
...
console.log("[Hunyuan3D] Bounding box preview extension registered");
