...
# Add xpart/partgen to path (standalone package structure)
_xpart_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'xpart', 'partgen'))
if _xpart_path not in sys.path:
    sys.path.insert(0, _xpart_path)

from models import sonata

# Import utils.misc using importlib to avoid any caching issues
_utils_misc_path = os.path.join(_xpart_path, 'utils', 'misc.py')
_spec = importlib.util.spec_from_file_location("utils.misc", _utils_misc_path)
_utils_misc = importlib.util.module_from_spec(_spec)
sys.modules['utils.misc'] = _utils_misc
_spec.loader.exec_module(_utils_misc)
smart_load_model = _utils_misc.smart_load_model
...
def build_P3SAM(self): #build p3sam
    ######################## Sonata ########################
    # Use ComfyUI's models folder for Sonata model
    try:
        import folder_paths
